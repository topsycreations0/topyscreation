...
import { backendUrl } from '../App'; // Importing backend URL
import { toast } from 'react-toastify'; // Importing toast for notifications

// Add Component: Handles adding a new product
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      const formData = new FormData(); // Create form data to send to the backend
      ...
      image1 && formData.append("image1", image1); // Append image1 if selected
      image2 && formData.append("image2", image2); // Append image2 if selected
      image3 && formData.append("image3", image3); // Append image3 if selected
      image4 && formData.append("image4", image4); // Append image4 if selected
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message); // Show error toast for unsuccessful response
      }
    } catch (error) {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      {/* Submit Button */}
      <button type='submit' className='w-28 py-3 mt-4 bg-black text-white active:bg-red-700'>ADD</button>

    </form>
  );
};

export default Add; // Exporting the Add component
